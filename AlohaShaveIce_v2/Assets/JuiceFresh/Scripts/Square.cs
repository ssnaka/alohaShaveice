...
public class Square : MonoBehaviour
{
    ...
    public Item GenItem (bool falling = true, int _col = -1, int _row = -1)
    {
        if (IsNone() && !CanGoInto())
            return null;

//        GameObject item = Instantiate(LevelManager.THIS.itemPrefab) as GameObject;
        GameObject item = LevelManager.Instance.GetItemFromPool();
        if (_col >= 0)
        {
            item.name = "Item_" + _col.ToString();
        }
        if (_row >= 0)
        {
            item.name = item.name + "_" + _row.ToString();
        }
